...
//------------------------------------------------------------------------------
static void
ImDrawListSplitter_Grow(ImDrawList* draw_list,
...
static void
ImDrawList_ChannelsGrow(ImDrawList* draw_list, int channels_count)
{
  ImDrawListSplitter_Grow(draw_list, &draw_list->_Splitter, channels_count);
}

static void
ImDrawListSplitter_SwapChannels(ImDrawListSplitter* splitter,
                                int left,
                                int right)
{
...
static void
ImDrawList_SwapChannels(ImDrawList* drawList, int left, int right)
{
  ImDrawListSplitter_SwapChannels(&drawList->_Splitter, left, right);
}

static void
ImDrawList_SwapSplitter(ImDrawList* drawList, ImDrawListSplitter& splitter)
{
...
static void
ImDrawList_PathBezierOffset(ImDrawList* drawList,
                            float offset,
                            const ImVec2& p0,
                            const ImVec2& p1,
                            const ImVec2& p2,
...
static void
ImDrawList_AddBezierWithArrows(ImDrawList* drawList,
...
//------------------------------------------------------------------------------
void
ed::Pin::Draw(ImDrawList* drawList, DrawFlags flags)
{
  if (flags & Hovered) {
...
ImVec2
ed::Pin::GetClosestPoint(const ImVec2& p) const
{
  return ImRect_ClosestPoint(m_Pivot, p, true, m_Radius + m_ArrowSize);
}

ImLine
ed::Pin::GetClosestLine(const Pin* pin) const
{
  return ImRect_ClosestLine(m_Pivot,
                            pin->m_Pivot,
                            m_Radius + m_ArrowSize,
...
void
ed::Node::UpdateDrag(const ImVec2& offset)
{
...
void
ed::Node::Draw(ImDrawList* drawList, DrawFlags flags)
{
  if (flags == Detail::Object::None) {
    ...
    if (IsGroup(this)) {
      ...
      if (m_GroupBorderWidth > 0.0f) {
        ...
        drawList->AddRect(m_GroupBounds.Min,
                          m_GroupBounds.Max,
                          m_GroupBorderColor,
                          m_GroupRounding,
                          15,
...
void
ed::Node::DrawBorder(ImDrawList* drawList, ImU32 color, float thickness)
{
  if (thickness > 0.0f) {
    drawList->AddRect(
      m_Bounds.Min, m_Bounds.Max, color, m_Rounding, 15, thickness);
  }
}

void
ed::Node::GetGroupedNodes(std::vector<Node*>& result, bool append)
{
  if (!append)
    result.resize(0);
...
ImRect
ed::Node::GetRegionBounds(NodeRegion region) const
{
...
ed::NodeRegion
ed::Node::GetRegion(const ImVec2& point) const
{
  if (m_Type == NodeType::Node) {
...
//------------------------------------------------------------------------------
void
ed::Link::Draw(ImDrawList* drawList, DrawFlags flags)
{
  if (flags == None) {
...
void
ed::Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
  if (!m_IsLive)
    return;
  ...
  ImDrawList_AddBezierWithArrows(drawList,
                                 curve,
...
bool
ed::Link::TestHit(const ImVec2& point, float extraThickness) const
{
  if (!m_IsLive)
    return false;
...
bool
ed::Link::TestHit(const ImRect& rect, bool allowIntersect) const
{
  if (!m_IsLive)
    return false;
...
void
ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
  if (!m_IsInitialized) {
    LoadSettings();
...
ed::EditorContext::End()
{
    ...
    auto copyNode = [&targetChannel, drawList](Node* node) {
      ...
      for (int i = 0; i < c_ChannelsPerNode; ++i)
        ImDrawList_SwapChannels(
          drawList, node->m_Channel + i, targetChannel + i);

      node->m_Channel = targetChannel;
      targetChannel += c_ChannelsPerNode;
    };

    auto groupsItEnd = std::find_if(m_Nodes.begin(),
                                    m_Nodes.end(),
  ...
  // Move user and hint channels to top
  {
    ...
    ImDrawList_SwapChannels(
      drawList, c_UserChannel_HintsBackground, channelCount + 0);
...
bool
ed::EditorContext::DoLink(LinkId id,
                          PinId startPinId,
                          PinId endPinId,
                          ImU32 color,
                          float thickness)
...
void
ed::EditorContext::SetNodePosition(NodeId nodeId, const ImVec2& position)
{
...
ImVec2
ed::EditorContext::GetNodePosition(NodeId nodeId)
{
  auto node = FindNode(nodeId);
  if (!node) 
  {
...
ImVec2
ed::EditorContext::GetNodeSize(NodeId nodeId)
{
  auto node = FindNode(nodeId);
  if (!node)
    return ImVec2(0, 0);
...
void
ed::EditorContext::MarkNodeToRestoreState(Node* node)
{
  node->m_RestoreState = true;
}

void
ed::EditorContext::RestoreNodeState(Node* node)
{
...
void
ed::EditorContext::ClearSelection()
{
  m_SelectedObjects.clear();
}

void
ed::EditorContext::SelectObject(Object* object)
{
  m_SelectedObjects.push_back(object);
}

void
ed::EditorContext::DeselectObject(Object* object)
{
  auto objectIt =
...
void
ed::EditorContext::SetSelectedObject(Object* object)
{
  ClearSelection();
  SelectObject(object);
}
...
void
ed::EditorContext::ToggleObjectSelection(Object* object)
{
...
bool
ed::EditorContext::IsSelected(Object* object)
{
  return std::find(m_SelectedObjects.begin(),
                   m_SelectedObjects.end(),
                   object) != m_SelectedObjects.end();
}

const ed::vector<ed::Object*>&
ed::EditorContext::GetSelectedObjects()
{
  return m_SelectedObjects;
}

bool
ed::EditorContext::IsAnyNodeSelected()
{
...
bool
ed::EditorContext::IsAnyLinkSelected()
{
  for (auto object : m_SelectedObjects)
    if (object->AsLink())
      return true;
...
bool
ed::EditorContext::HasSelectionChanged()
{
  return m_LastSelectedObjects != m_SelectedObjects;
}

ed::Node*
ed::EditorContext::FindNodeAt(const ImVec2& p)
{
  for (auto node : m_Nodes)
    if (node->TestHit(p))
      return node;
...
void
ed::EditorContext::FindNodesInRect(const ImRect& r,
                                   vector<Node*>& result,
                                   bool append,
...
void
ed::EditorContext::FindLinksInRect(const ImRect& r,
                                   vector<Link*>& result,
                                   bool append)
{
  if (!append)
...
void
ed::EditorContext::FindLinksForNode(NodeId nodeId,
                                    vector<Link*>& result,
                                    bool add)
{
  if (!add)
...
bool
ed::EditorContext::PinHadAnyLinks(PinId pinId)
{
...
void
ed::EditorContext::NotifyLinkDeleted(Link* link)
{
...
void
ed::EditorContext::Suspend(SuspendFlags flags)
{
  auto drawList = ImGui::GetWindowDrawList();
...
void
ed::EditorContext::Resume(SuspendFlags flags)
{
  auto drawList = ImGui::GetWindowDrawList();
  if ((flags & SuspendFlags::KeepSplitter) != SuspendFlags::KeepSplitter)
    ImDrawList_SwapSplitter(drawList, m_Splitter);
...
bool
ed::EditorContext::IsSuspended()
{
  return m_Canvas.IsSuspended();
}

bool
ed::EditorContext::IsActive()
{
  return m_IsWindowActive;
}

ed::Pin*
ed::EditorContext::CreatePin(PinId id, PinKind kind)
{
...
ed::Node*
ed::EditorContext::CreateNode(NodeId id)
{
  IM_ASSERT(nullptr == FindObject(id));
...
ed::Link*
ed::EditorContext::CreateLink(LinkId id)
{
  IM_ASSERT(nullptr == FindObject(id));
...
static inline auto
FindItemInLinear(C& container, Id id)
{
#if defined(_DEBUG)
...
ed::Node*
ed::EditorContext::FindNode(NodeId id)
{
  return FindItemInLinear(m_Nodes, id);
}

ed::Pin*
ed::EditorContext::FindPin(PinId id)
{
  return FindItemIn(m_Pins, id);
}

ed::Link*
ed::EditorContext::FindLink(LinkId id)
{
  return FindItemIn(m_Links, id);
}

ed::Object*
ed::EditorContext::FindObject(ObjectId id)
{
  if (id.IsNodeId())
...
ed::Node*
ed::EditorContext::GetNode(NodeId id)
{
...
ed::Pin*
ed::EditorContext::GetPin(PinId id, PinKind kind)
{
...
ed::Link*
ed::EditorContext::GetLink(LinkId id)
{
...
void
ed::EditorContext::MakeDirty(SaveReasonFlags reason)
{
  m_Settings.MakeDirty(reason);
}

void
ed::EditorContext::MakeDirty(SaveReasonFlags reason, Node* node)
{
  m_Settings.MakeDirty(reason, node);
}

ed::Link*
ed::EditorContext::FindLinkAt(const ImVec2& p)
{
  for (auto& link : m_Links)
...
ImU32
ed::EditorContext::GetColor(StyleColor colorIndex) const
{
  return ImColor(m_Style.Colors[colorIndex]);
}

ImU32
ed::EditorContext::GetColor(StyleColor colorIndex, float alpha) const
{
...
void
ed::EditorContext::RegisterAnimation(Animation* animation)
{
  m_LiveAnimations.push_back(animation);
}

void
ed::EditorContext::UnregisterAnimation(Animation* animation)
{
  auto it =
...
void
ed::EditorContext::UpdateAnimations()
{
...
void
ed::EditorContext::Flow(Link* link)
{
  m_FlowAnimationController.Flow(link);
}

void
ed::EditorContext::SetUserContext(bool globalSpace)
{
...
void
ed::EditorContext::EnableShortcuts(bool enable)
{
  m_ShortcutsEnabled = enable;
}

bool
ed::EditorContext::AreShortcutsEnabled()
{
  return m_ShortcutsEnabled;
}

ed::Control
ed::EditorContext::BuildControl(bool allowOffscreen)
{
  if (!allowOffscreen &&
      !ImGui::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
    return Control(
      nullptr, nullptr, nullptr, nullptr, false, false, false, false);
...
void
ed::EditorContext::ShowMetrics(const Control& control)
{
  auto& io = ImGui::GetIO();
  ...
  auto liveNodeCount = (int)std::count_if(
  ...
  auto liveLinkCount = (int)std::count_if(
    m_Links.begin(), m_Links.end(), [](Link* link) { return link->m_IsLive; });
  ...
  ImGui::Text("Canvas Size: { w=%g h=%g }",
              canvasRect.GetWidth(),
              canvasRect.GetHeight());
  ImGui::Text("Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }",
              localMousePos.x,
              localMousePos.y,
              globalMousePos.x,
              globalMousePos.y);
...
void
ed::NodeSettings::MakeDirty(SaveReasonFlags reason)
{
  m_IsDirty = true;
...
bool
ed::NodeSettings::Parse(const std::string& string, NodeSettings& settings)
{
...
bool
ed::NodeSettings::Parse(const json::value& data, NodeSettings& result)
{
  if (!data.is_object())
    return false;
...
ed::NodeSettings*
ed::Settings::FindNode(NodeId id)
{
...
void
ed::Settings::ClearDirty(Node* node)
{
  if (node) {
...
void
ed::Settings::MakeDirty(SaveReasonFlags reason, Node* node)
{
  m_IsDirty = true;
  m_DirtyReason = m_DirtyReason | reason;
...
bool
ed::Settings::Parse(const std::string& string, Settings& settings)
{
...
void
ed::Animation::Play(float duration)
{
  if (IsPlaying())
    Stop();
...
void
ed::NavigateAnimation::NavigateTo(const ImRect& target, float duration)
{
  Stop();
...
void
ed::NavigateAnimation::OnUpdate(float progress)
{
  ImRect current;
  current.Min =
...
void
ed::NavigateAnimation::OnStop()
{
  Editor->MakeDirty(SaveReasonFlags::Navigation);
}

void
ed::NavigateAnimation::OnFinish()
{
...
void
ed::FlowAnimation::Flow(ed::Link* link,
...
void
ed::FlowAnimation::Draw(ImDrawList* drawList)
{
...
bool
ed::FlowAnimation::IsLinkValid() const
{
  return m_Link && m_Link->m_IsLive;
}

bool
ed::FlowAnimation::IsPathValid() const
{
...
ImVec2
ed::FlowAnimation::SamplePath(float distance)
{
  // distance = ImMax(0.0f, std::min(distance, PathLength));

  auto endPointIt =
    std::find_if(m_Path.begin(), m_Path.end(), [distance](const CurvePoint& p) {
...
void
ed::FlowAnimation::OnUpdate(float progress)
{
  IM_UNUSED(progress);
...
void
ed::FlowAnimationController::Flow(Link* link)
{
  ...
  animation->Flow(link,
                  ...
                  editorStyle.FlowSpeed,
                  editorStyle.FlowDuration);
}

void
ed::FlowAnimationController::Draw(ImDrawList* drawList)
{
...
ed::FlowAnimation*
ed::FlowAnimationController::GetOrCreate(Link* link)
{
...
void
ed::FlowAnimationController::Release(FlowAnimation* animation)
{
  IM_UNUSED(animation);
}
  ...
  if (ImGui::IsWindowFocused() && ImGui::IsKeyPressed(GetKeyIndexForF()) &&
    ...
    if (!Editor->GetSelectedObjects().empty()) {
      if (m_Reason != NavigationReason::Selection ||
        ...
        NavigateTo(Editor->GetSelectionBounds(),
                   allowZoomIn,
                   -1.0f,
    ...
    if (navigateToContent)
      NavigateTo(
        Editor->GetContentBounds(), true, -1.0f, NavigationReason::Content);
  }
...
bool
ed::NavigateAction::Process(const Control& control)
{
  IM_UNUSED(control);
...
bool
ed::NavigateAction::HandleZoom(const Control& control)
{
  IM_UNUSED(control);
...
void
ed::NavigateAction::NavigateTo(const ImRect& bounds,
                               bool zoomIn,
                               float duration,
...
void
ed::NavigateAction::NavigateTo(const ImRect& target,
                               float duration,
                               NavigationReason reason)
{
  m_Reason = reason;
...
void
ed::NavigateAction::StopNavigation()
{
  m_Animation.Stop();
}

void
ed::NavigateAction::FinishNavigation()
{
  m_Animation.Finish();
}

bool
ed::NavigateAction::MoveOverEdge()
{
...
void
ed::NavigateAction::StopMoveOverEdge()
{
  if (m_MovingOverEdge) {
    Editor->MakeDirty(SaveReasonFlags::Navigation);
...
void
ed::NavigateAction::SetWindow(ImVec2 position, ImVec2 size)
{
...
ImGuiEx::CanvasView
ed::NavigateAction::GetView() const
{
  return ImGuiEx::CanvasView(-m_Scroll, m_Zoom);
}

ImVec2
ed::NavigateAction::GetViewOrigin() const
{
  return -m_Scroll;
}

float
ed::NavigateAction::GetViewScale() const
{
  return m_Zoom;
}

void
ed::NavigateAction::SetViewRect(const ImRect& rect)
{
...
ImRect
ed::NavigateAction::GetViewRect() const
{
  return m_Canvas.CalcViewRect(GetView());
}

float
ed::NavigateAction::MatchZoom(int steps, float fallbackZoom)
{
...
int
ed::NavigateAction::MatchZoomIndex(int direction)
{
  int bestIndex = -1;
...
ed::EditorAction::AcceptResult
ed::SizeAction::Accept(const Control& control)
{
  IM_ASSERT(!m_IsActive);
...
bool
ed::SizeAction::Process(const Control& control)
{
  if (m_Clean) {
    m_Clean = false;
...
ed::SizeAction::ShowMetrics()
{
  ...
  if (m_SizedNode && m_IsActive) {
    ...
    ImGui::Text(
      "    Minimum Size: { w=%g h=%g }", m_MinimumSize.x, m_MinimumSize.y);
    ImGui::Text("    Last Size: { w=%g h=%g }", m_LastSize.x, m_LastSize.y);
  }
}

ed::NodeRegion
ed::SizeAction::GetRegion(Node* node)
{
  return node->GetRegion(ImGui::GetMousePos());
}

ImGuiMouseCursor
ed::SizeAction::ChooseCursor(NodeRegion region)
{
  switch (region) {
    default:
...
ed::EditorAction::AcceptResult
ed::DragAction::Accept(const Control& control)
{
  IM_ASSERT(!m_IsActive);
...
bool
ed::DragAction::Process(const Control& control)
{
  if (m_Clear) {
    m_Clear = false;
...
bool
ed::SelectAction::Process(const Control& control)
{
  IM_UNUSED(control);
...
void
ed::SelectAction::Draw(ImDrawList* drawList)
{
...
bool
ed::ContextMenuAction::Process(const Control& control)
{
  IM_UNUSED(control);
...
void
ed::ContextMenuAction::ShowMetrics()
{
  EditorAction::ShowMetrics();
...
bool
ed::ContextMenuAction::ShowNodeContextMenu(NodeId* nodeId)
{
...
bool
ed::ContextMenuAction::ShowPinContextMenu(PinId* pinId)
{
...
bool
ed::ContextMenuAction::ShowLinkContextMenu(LinkId* linkId)
{
...
bool
ed::ContextMenuAction::ShowBackgroundContextMenu()
{
  ...
  if (candidateAction != None) {
    if (candidateAction != Paste && candidateAction != CreateNode) {
          ...
          m_Context.size() > 1) // one node cannot make connection to anything
      {
        ...
        std::for_each(
          m_Context.begin(), m_Context.end(), [&nodes](Object* object) {
            if (auto node = object->AsNode())
...
bool
ed::ShortcutAction::Process(const Control& control)
{
  IM_UNUSED(control);
...
bool
ed::ShortcutAction::AcceptDuplicate()
{
  IM_ASSERT(m_InAction);
...
bool
ed::ShortcutAction::AcceptCreateNode()
{
  IM_ASSERT(m_InAction);
...
bool
ed::CreateItemAction::Process(const Control& control)
{
  IM_ASSERT(m_IsActive);
...
void
ed::CreateItemAction::ShowMetrics()
{
  EditorAction::ShowMetrics();
...
void
ed::CreateItemAction::SetStyle(ImU32 color, float thickness)
{
  m_LinkColor = color;
...
void
ed::CreateItemAction::DragStart(Pin* startPin)
{
  IM_ASSERT(!m_InActive);
...
void
ed::CreateItemAction::DropPin(Pin* endPin)
{
  IM_ASSERT(!m_InActive);
...
void
ed::CreateItemAction::DropNothing()
{
  IM_ASSERT(!m_InActive);
...
ed::CreateItemAction::Result
ed::CreateItemAction::RejectItem()
{
  IM_ASSERT(m_InActive);
...
ed::CreateItemAction::Result
ed::CreateItemAction::AcceptItem()
{
  IM_ASSERT(m_InActive);
...
ed::CreateItemAction::Result
ed::CreateItemAction::QueryLink(PinId* startId, PinId* endId)
{
  IM_ASSERT(m_InActive);
...
ed::CreateItemAction::Result
ed::CreateItemAction::QueryNode(PinId* pinId)
{
  IM_ASSERT(m_InActive);
  ...
  auto addDeadLinks = [this]() {
    ...
    if (!links.empty()) {
      ...
      m_CandidateObjects.insert(
        m_CandidateObjects.end(), links.begin(), links.end());
    }
  };
...
bool
ed::DeleteItemsAction::Process(const Control& control)
{
  IM_UNUSED(control);
...
void
ed::DeleteItemsAction::ShowMetrics()
{
  EditorAction::ShowMetrics();
...
bool
ed::DeleteItemsAction::Add(Object* object)
{
...
bool
ed::DeleteItemsAction::QueryLink(LinkId* linkId, PinId* startId, PinId* endId)
{
  ObjectId objectId;
...
bool
ed::DeleteItemsAction::QueryNode(NodeId* nodeId)
{
  ObjectId objectId;
...
bool
ed::DeleteItemsAction::QueryItem(ObjectId* itemId, IteratorType itemType)
{
  if (!m_InInteraction)
    return false;
...
bool
ed::DeleteItemsAction::AcceptItem()
{
  if (!m_InInteraction)
    return false;
...
void
ed::DeleteItemsAction::RejectItem()
{
  if (!m_InInteraction)
    return;
...
void
ed::DeleteItemsAction::RemoveItem()
{
  auto item = m_CandidateObjects[m_CandidateItemIndex];
...
void
ed::NodeBuilder::Begin(NodeId nodeId)
{
...
void
ed::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
  IM_ASSERT(nullptr != m_CurrentNode);
...
ed::NodeBuilder::EndPin()
{
  ...
  if (m_ResolvePinRect) {
    ...
    IM_ASSERT(m_CurrentPin->m_Bounds.GetSize().x != 0.0f &&
              m_CurrentPin->m_Bounds.GetSize().y != 0.0f);
  }
...
void
ed::NodeBuilder::PinRect(const ImVec2& a, const ImVec2& b)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void
ed::NodeBuilder::PinPivotRect(const ImVec2& a, const ImVec2& b)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void
ed::NodeBuilder::PinPivotSize(const ImVec2& size)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void
ed::NodeBuilder::PinPivotScale(const ImVec2& scale)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void
ed::NodeBuilder::PinPivotAlignment(const ImVec2& alignment)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void
ed::NodeBuilder::Group(const ImVec2& size)
{
  IM_ASSERT(nullptr != m_CurrentNode);
  IM_ASSERT(nullptr == m_CurrentPin);
  IM_ASSERT(false == m_IsGroup);
...
ImDrawList*
ed::NodeBuilder::GetUserBackgroundDrawList() const
{
  return GetUserBackgroundDrawList(m_CurrentNode);
}

ImDrawList*
ed::NodeBuilder::GetUserBackgroundDrawList(Node* node) const
{
...
bool
ed::HintBuilder::Begin(NodeId nodeId)
{
  IM_ASSERT(nullptr == m_CurrentNode);
...
ImDrawList*
ed::HintBuilder::GetForegroundDrawList()
{
  IM_ASSERT(nullptr != m_CurrentNode);
...
ImDrawList*
ed::HintBuilder::GetBackgroundDrawList()
{
  IM_ASSERT(nullptr != m_CurrentNode);
...
//------------------------------------------------------------------------------
void
ed::Style::PushColor(StyleColor colorIndex, const ImVec4& color)
{
...
void
ed::Style::PushVar(StyleVar varIndex, float value)
{
...
void
ed::Style::PushVar(StyleVar varIndex, const ImVec2& value)
{
...
void
ed::Style::PushVar(StyleVar varIndex, const ImVec4& value)
{
...
const char*
ed::Style::GetColorName(StyleColor colorIndex) const
{
  switch (colorIndex) {
    case StyleColor_Bg:
      return "Bg";
...
float*
ed::Style::GetVarFloatAddr(StyleVar idx)
{
  switch (idx) {
...
ImVec4*
ed::Style::GetVarVec4Addr(StyleVar idx)
{
  switch (idx) {
...
std::string
ed::Config::LoadNode(NodeId nodeId)
{
  std::string data;
...
bool
ed::Config::Save(const std::string& data, SaveReasonFlags flags)
{
  if (SaveSettings) {
...
bool
ed::Config::SaveNode(NodeId nodeId,
