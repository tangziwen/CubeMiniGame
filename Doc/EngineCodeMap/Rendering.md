...
- `RenderGraph`: V1 graph runner for sequential graph passes and imported/owned resources; `RenderGraphResourceHandle` names textures/framebuffers, `createFrameBuffer()` creates graph-owned framebuffer resources through the active backend, and bound stages are added to `RenderPath` after execution.
...
- `SceneView`: main 3D view that owns the GBuffer, deferred lighting, transparent, sky, debug wireframe, and a `RenderGraph`-sequenced post chain; its core scene/GBuffer/HBAO/FXAA framebuffers are now graph-owned V1 resources.
