...
- `RenderBackEndBase`: abstract factory and frame interface for textures, shaders, buffers, render passes, pipelines, stages, framebuffers, materials, frame begin/end, current frame command and swapchain framebuffer access.
...
- `DeviceRenderStage`: backend-neutral stage interface for render/compute passes, binding, draw calls, screen quad/sphere helpers, per-stage pipeline/material caches, neutral custom vertex layouts, 16/32-bit indexed draws and view camera propagation to material uniforms.
