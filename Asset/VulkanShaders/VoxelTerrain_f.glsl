...
layout(location = 4) in vec3 v_matBlend;
layout(location = 5) in float[16] v_mat;
layout(location = 22) in vec3 v_worldPos;
...
vec4 triplanarSample(sampler2D samp, float texIndex, float scaleFactor)
{
  ...
  vec2 xPlane = getUV(v_worldPos.yz * scaleFactor, index,dx_x,dy_x);
	...
	vec4 xaxis = textureGrad( samp, xPlane, dx_x, dy_x);
  vec2 yPlane = getUV(v_worldPos.xz * scaleFactor,index, dx_y, dy_y);
	vec4 yaxis = textureGrad( samp, yPlane, dx_y, dy_y);
  vec2 zPlane = getUV(v_worldPos.xy * scaleFactor,index, dx_z, dy_z);
	vec4 zaxis = textureGrad( samp, zPlane, dx_z, dy_z);
