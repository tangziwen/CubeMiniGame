...
BearPart*
BuildingSystem::placeBearingToAttach(Attachment* attachment)
{
  vec3 pos, n, up;
  ...
  if (doc.HasMember("constraintList")) {
    ...
    for (unsigned int i = 0; i < items.Size(); i++) {
      ...
      if (item.HasMember("to")) {
        ...
        // toAttach->m_parent->attachToFromOtherIslandAlterSelfIsland(fromAttach,
        // toAttach);
      }
    }
  }
...
void
BuildingSystem::replaceToLift(Island* island)
{
  if (island) {
    vec3 attachPos, n, up;
    ...
    for (auto part : island->m_partList) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (attach->m_parent->isConstraint()) {
          ...
          other->m_parent->attachToFromOtherIslandAlterSelfIsland(attach,
                                                                  other);
        }
      }
    }
...
// toDo
void
BuildingSystem::findPiovtAndAxis(Attachment* attach,
                                 vec3 hingeDir,
                                 vec3& pivot,
                                 vec3& asix)
{
...
void
BuildingSystem::tmpMoveWheel(bool isOpen)
{
